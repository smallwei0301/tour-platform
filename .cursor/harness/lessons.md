...
## [2026-08-05] paid-at-is-not-proof-of-money
- Context：#1777 收尾階段，我基於「14 筆 `paid` 卡單是真實歷史訂單」提出 DML runbook，建議把它們推回 `confirmed`，預期 NT$20,261 入導遊餘額。owner 一句「這些其實都是測試資料」把整份提案翻掉
- Error：我的候選準則只看 `orders.paid_at IS NOT NULL`，並把它當成「provider 確實已收款」。**`paid_at` 只是 orders 表上的一個時間戳，測試路徑照樣寫得進去**。真正的實收證據是 `payment_events` 有對應列——實測那 14 筆與撐起 NT$21,814 餘額的 10 筆 `payout_items`，`payment_events` 全部是 0
- 第二層錯誤（更值得記）：我把「三方對帳無差異：ledger = 餘額 = NT$21,814」寫進 runbook 當放行前提。那句話**技術上為真但當作準備度訊號時是誤導**——測試資料自己跟自己對得起來，對真實金流零證據力。內部一致性 ≠ 對外正確性
- 第三層錯誤：我把「出款 HOLD 未解除」當成項目一的安全網。安全網只在「錢是真的、只是時機未到」時成立；錢是假的時候那不是安全網，只是把錯誤延後到解除 HOLD 的那一刻引爆
- Solution：(a) 任何碰錢的歷史資料處置，候選準則**必須**含 `EXISTS (SELECT 1 FROM payment_events pe WHERE pe.order_id = o.id)`，不得只憑 `paid_at`／`status`；(b) 提「這筆錢該進帳」之前先問一句「平台真的收到這筆錢了嗎，證據在哪張表」；(c) 冷啟動期的 production 預設**混有測試資料**，任何彙總數字在被當成準備度訊號前，先驗證資料來源真偽（seed UUID 如 `22222222…`／`00000000…` 是強訊號）
- 附帶發現：收尾時另抓到 2 筆 `completed` 但無 guide／無 `eligible_since` 的訂單，會讓 `reconciliation.ok` **恆為 false**。永遠紅著的燈和永遠綠著的燈一樣沒用，都會蓋掉未來的真實異常——加稽核桶時要順便確認它有辦法回到綠燈
- 適用範圍：所有歷史資料修復提案、對帳結論的解讀、上線準備度判斷
