...
relation_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'relation:' || n.nspname || '.' || c.relname,
    'schema', n.nspname,
    'name', c.relname,
    'kind', c.relkind,
...
column_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'column:' || n.nspname || '.' || c.relname || '.' || a.attname,
    'schema', n.nspname,
    'relation', c.relname,
    'position', a.attnum,
    'name', a.attname,
...
type_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'type:' || n.nspname || '.' || t.typname,
    'schema', n.nspname,
    'name', t.typname,
    'kind', t.typtype,
...
constraint_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'constraint:' || n.nspname || '.' || c.relname || '.' || con.conname,
    'schema', n.nspname,
    'relation', c.relname,
    'name', con.conname,
    'type', con.contype,
...
index_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'index:' || n.nspname || '.' || i.relname,
    'schema', n.nspname,
    'relation', c.relname,
    'name', i.relname,
...
routine_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'routine:' || n.nspname || '.' || p.proname || '(' || pg_catalog.pg_get_function_identity_arguments(p.oid) || ')',
    'schema', n.nspname,
    'name', p.proname,
...
trigger_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'trigger:' || n.nspname || '.' || c.relname || '.' || t.tgname,
    'schema', n.nspname,
    'relation', c.relname,
    'name', t.tgname,
    'enabled', t.tgenabled,
...
rls_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'rls:' || n.nspname || '.' || c.relname,
    'schema', n.nspname,
    'relation', c.relname,
...
policy_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'policy:' || n.nspname || '.' || c.relname || '.' || p.polname,
    'schema', n.nspname,
    'relation', c.relname,
    'name', p.polname,
    ...
    'command', p.polcmd,
    'roles', (
      SELECT jsonb_agg(pg_catalog.pg_get_userbyid(role_id) ORDER BY pg_catalog.pg_get_userbyid(role_id))
...
acl_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'acl:' || s.object_type || ':' || s.schema_name || '.' || s.object_name || ':' ||
      pg_catalog.pg_get_userbyid(a.grantor) || ':' || pg_catalog.pg_get_userbyid(a.grantee) || ':' ||
    ...
    'grantee', pg_catalog.pg_get_userbyid(a.grantee),
...
owner_rows AS (
  SELECT jsonb_build_object('canonicalKey', 'owner:schema:' || n.nspname, 'objectType', 'schema', 'schema', n.nspname, 'object', n.nspname, 'owner', pg_catalog.pg_get_userbyid(n.nspowner)) AS item
  FROM eligible_namespaces AS n
  UNION ALL
  SELECT jsonb_build_object('canonicalKey', 'owner:relation:' || n.nspname || '.' || c.relname, 'objectType', 'relation', 'schema', n.nspname, 'object', c.relname, 'owner', pg_catalog.pg_get_userbyid(c.relowner))
  FROM pg_catalog.pg_class AS c JOIN eligible_namespaces AS n ON n.oid = c.relnamespace
  WHERE c.relkind IN ('r', 'p', 'v', 'm', 'S', 'f')
  UNION ALL
  SELECT jsonb_build_object('canonicalKey', 'owner:routine:' || n.nspname || '.' || p.proname || '(' || pg_catalog.pg_get_function_identity_arguments(p.oid) || ')', 'objectType', 'routine', 'schema', n.nspname, 'object', p.proname || '(' || pg_catalog.pg_get_function_identity_arguments(p.oid) || ')', 'owner', pg_catalog.pg_get_userbyid(p.proowner))
  FROM pg_catalog.pg_proc AS p JOIN eligible_namespaces AS n ON n.oid = p.pronamespace
  UNION ALL
  SELECT jsonb_build_object('canonicalKey', 'owner:type:' || n.nspname || '.' || t.typname, 'objectType', 'type', 'schema', n.nspname, 'object', t.typname, 'owner', pg_catalog.pg_get_userbyid(t.typowner))
  FROM pg_catalog.pg_type AS t JOIN eligible_namespaces AS n ON n.oid = t.typnamespace
  WHERE t.typtype IN ('e', 'd', 'c', 'r', 'm')
),
default_privilege_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'default-privilege:' || pg_catalog.pg_get_userbyid(d.defaclrole) || ':' || COALESCE(n.nspname, '*') || ':' || d.defaclobjtype::text || ':' || pg_catalog.pg_get_userbyid(a.grantor) || ':' || pg_catalog.pg_get_userbyid(a.grantee) || ':' || a.privilege_type || ':' || a.is_grantable::text,
...
extension_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'extension:' || e.extname,
    'name', e.extname,
...
publication_rows AS (
  SELECT jsonb_build_object(
    'canonicalKey', 'publication:' || p.pubname,
  ...
  UNION ALL
  SELECT jsonb_build_object(
    'canonicalKey', 'publication:' || p.pubname || ':relation:' || n.nspname || '.' || c.relname,
    'membershipKind', 'relation',
    'publication', p.pubname,
    'schema', n.nspname,
    'relation', c.relname,
    'rowFilter', pg_catalog.pg_get_expr(pr.prqual, pr.prrelid),
  ...
  UNION ALL
  SELECT jsonb_build_object(
    'canonicalKey', 'publication:' || p.pubname || ':schema:' || n.nspname,
...
managed_overlay_source AS (
  SELECT 'relation'::text AS overlay_kind, item FROM relation_rows
...
managed_overlay_rows AS (
  SELECT (s.item - 'canonicalKey') || jsonb_build_object(
    'canonicalKey', 'overlay:' || s.overlay_kind || ':' || (s.item ->> 'canonicalKey'),
    'sourceCanonicalKey', s.item ->> 'canonicalKey',
    'overlayKind', s.overlay_kind
  ) AS item
  FROM managed_overlay_source AS s
  WHERE s.item ->> 'schema' IN ('auth', 'storage', 'realtime', 'extensions', 'graphql', 'supabase_functions', 'vault')
)
SELECT jsonb_build_object(
  'schemaVersion', 1,
  'extractorVersion', 1,
  ...
  'sections', jsonb_build_object(
    ...
    'managedSchemaOverlays', COALESCE((SELECT jsonb_agg(item ORDER BY item ->> 'canonicalKey') FROM managed_overlay_rows), '[]'::jsonb)
  )
);
