...
// 1200×810：三欄兩列，欄寬 400、列高 405。bounds = { x, y, width, height }。
// 版型對應選單圖（上排：我的訂單／前往付款／探索行程；下排：我的帳號／我的收藏／常見問題）。
const COL = 400;
const ROW = 405;
...
const richMenu = {
  ...
  chatBarText: '選單',
  areas: [
    // 上排：前兩格用「文字動作」→ 觸發免費 Reply 訂單查詢。
    { bounds: cell(0, 0), action: { type: 'message', text: '我的訂單' } },                  // 我的訂單
