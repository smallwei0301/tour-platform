...
test('book step1：示範嚮導 3 張方案卡（對齊 mockup 圖2），可選第 2 張並進 step2', async ({ page }) => {
  ...
  // 對齊 mockup 圖2 的分組：屏東縣 1 卡（1 行標題）＋ 台東縣 2 卡（同活動 2 方案）＝ 共 3 卡
  ...
  await expect(page.getByText('晨霧稜線半日健行')).toBeVisible();
  await expect(page.getByText('祕境海灣浮潛｜退潮才現身的珊瑚礁灣').first()).toBeVisible();

  // 選第 2 張（台東縣首張）→ 出現同行人數 stepper、底部摘要；按 CTA 進 step2 選日期
