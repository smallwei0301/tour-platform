...
async function installRoutes(page: Page): Promise<FakeState> {
  ...
  await page.route('**/api/v2/admin/payouts/balances', async (route: Route) => {
    const balances = [
      {
  ...
  await page.route('**/api/v2/admin/payouts/generate', async (route: Route) => {
  ...
  await page.route(`**/api/v2/admin/payouts/${P1}/cancel`, async (route: Route) => {
    const req = route.request();
  ...
  // List endpoint — exact match only ('**/api/v2/admin/payouts' does not catch
