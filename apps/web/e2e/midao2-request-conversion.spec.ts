...
test('midao2 reloads the canonical projection after a conversion conflict', async ({ page }) => {
  ...
  await page.route(`**/api/v2/guide/midao/requests/${REQUEST_ID}`, async (route) => {
    if (route.request().method() !== 'GET') writes.push(new URL(route.request().url()).pathname);
    requestReads += 1;
    const converted = requestReads > 1;
    await route.fulfill({
      contentType: 'application/json',
      body: JSON.stringify({
        success: true,
        data: {
          ...
          canonicalInquiry: {
            inquiryId: INQUIRY_ID, status: converted ? 'converted' : 'replied',
            convertedBookingId: converted ? 'dddddddd-dddd-4ddd-8ddd-dddddddddddd' : null,
            plan: {
              activityPlanId: PLAN_ID, name: '測試方案', bookingType: 'request', status: 'active',
              minParticipants: 1, maxParticipants: 6, basePrice: 1800,
            },
  ...
  await page.route(`**/api/v2/guide/inquiries/${INQUIRY_ID}/commands/convert`, async (route) => {
    ...
    await route.fulfill({
      status: 409,
      contentType: 'application/json',
      body: JSON.stringify({ success: false, error: { code: 'INQUIRY_ALREADY_CONVERTED' } }),
    });
  });
  ...
  await expect(page.getByTestId('midao-conversion-reload')).toBeVisible();
  await page.getByTestId('midao-conversion-reload').click();
  await expect(page.getByTestId('midao2-canonical-converted')).toBeVisible();
