...
// 經典首頁（原 `/`）：#1713 起 `/` 改為 3D 滾動首頁「祕島世界」，本頁搬至 /home，
// 供世界頁 CTA 與導覽列品牌 logo 連回。裸 /home 由 next.config beforeFiles rewrite
// 導入 [locale]（middleware matcher 未涵蓋 /home；/en/home 等前綴路徑則直接命中）。
...
export async function generateMetadata(
  ...
  return {
    title: 'Midao 祕島｜台灣在地嚮導預約平台',
    description: '找到懂路的人，帶你走進台灣最有故事的地方。柴山探洞、大稻埕老街、花蓮溯溪⋯⋯ 預約實名認證在地嚮導，安全透明。',
    alternates: buildAlternates('/home', locale),
    openGraph: {
      title: 'Midao 祕島｜台灣在地嚮導預約平台',
      description: '找到懂路的人，帶你走進台灣最有故事的地方。',
      images: [{ url: '/images/og-default.png', width: 1536, height: 1024, alt: 'Midao 祕島｜台灣在地嚮導預約平台' }],
    },
    twitter: {
...
export default async function HomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  // Organization／WebSite JSON-LD 隨首頁職責移到 `/`（世界頁）；
  // 本頁保留與可見 FAQ 內容對應的 FAQPage。
  const homeJsonLd = {
