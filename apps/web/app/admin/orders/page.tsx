...
const STATUS_EFFECTS: Record<string, string> = {
  ...
  paid:
    ...
    '\n⏩ 自動推進（#1637）：線上 ECPay 付款的 V2 訂單，付款當下即自動跳到「已確認」，通常不會停在此狀態。仍停在「已付款」的通常是：修復前的舊訂單、admin POS 手動收款單、或方案 booking_type 未設定——這些**不會**被自動完成／結算掃到，需人工切換到「已確認」才會進入自動化鏈。' +
    '\n✏️ 可繼續編輯狀態、備註、聯絡方式、人數等欄位。尚未進入出帳。',

  confirmed:
    '已確認名額，自動完成鏈的入口狀態（線上付款成功的 V2 訂單自 #1637 起付款當下自動到達此狀態）。' +
    '\n⏩ 自動轉「已完成」的條件（滿足其一）：' +
    '\n　(a) 出團時間（V2 依 booking 出發時間，舊單退回場次時間）過後滿 48 小時，由每日 02:30（台北）auto-complete sweep 自動完成；' +
    '\n　(b) 導遊掃描旅客的電子憑證 QR／輸入短碼核銷，立即提前完成。' +
    '\n⚠️ 訂單若缺出團時間，sweep 不會自動完成（會列入 stalled 告警），需人工切「已完成」。' +
  ...
  completed:
    '行程已完成，最後一個「活躍」狀態。到達方式：自動（出團後 48 小時 sweep／導遊掃碼核銷）或此處手動切換。' +
    '\n💰 出帳資格：唯一被結算 sweep（/api/internal/settlement/sweep）納入候選的狀態。需同時滿足：paid_at 有值（實際收到款）、出團後滿 T+7 天、無退款、無投訴／爭議／安全 hold 才會結算；滿足後導遊 payout = (總額 - 已退款) × 85%，結算入導遊餘額（每日 10:00 台北）。' +
