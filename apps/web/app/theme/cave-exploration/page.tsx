    ...
    <main>
      ...
      <section className="tp-theme-hero tp-theme-cave-hero">
        <div className="tp-container">
          ...
          <Link className="tp-btn tp-btn-primary" href="/activities?type=%E6%9F%B4%E5%B1%B1%E6%8E%A2%E6%B4%9E">探索柴山行程</Link>
        </div>
      </section>
          ...
          <div className="tp-card-grid tp-card-grid-activities">
            {caveTours.map((t) => (
              <article className="tp-card" key={t.slug + t.title}>
                ...
                <h3>{t.title}</h3>
                <p>{t.meta}</p>
                <Link className="tp-link" href={`/activities/kaohsiung/${encodeURIComponent(t.slug)}`}>查看行程 →</Link>
              </article>
            ))}
          </div>
        </div>
