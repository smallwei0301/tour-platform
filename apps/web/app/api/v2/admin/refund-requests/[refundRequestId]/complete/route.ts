/**
 * #1649 Phase 6：實作自 legacy 路徑（app/api/admin/refund-requests/[refundRequestId]/complete）整體搬遷至 v2 命名空間。
...
export async function POST(request: Request, context: { params: Promise<{ refundRequestId: string }> }) {
  ...
  try {
    const row = await updateAdminRefundStatusDb({ refundRequestId, action: 'complete', adminNote: body?.adminNote });
    return Response.json(ok(row));
  } catch (err) {
    // #1598：未預期例外上報（fire-and-forget，不改變回應行為）。
    void reportRouteError(err, { route: 'v2/admin/refund-requests/[refundRequestId]/complete' });
