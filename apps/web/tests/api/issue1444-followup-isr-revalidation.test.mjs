...
test('首頁精選路由：PUT 成功後 revalidatePath(\'/\') 立即失效首頁 ISR', async () => {
  ...
  assert.match(src, /revalidatePath\(\s*['"]\/['"]\s*\)/, 'PUT 成功後應 revalidatePath("/")');
  // best-effort：失敗不得擋下 admin 操作
  assert.match(src, /try\s*\{[\s\S]*revalidatePath\(\s*['"]\/['"][\s\S]*catch/, 'revalidatePath 應包在 try/catch');
});
