...
test('checkout 預填: checkout page 接 /api/me/profile 並只在欄位為空時帶入', () => {
  const src = readFileSync(path.resolve('app/checkout/page.tsx'), 'utf8');
  assert.match(src, /\/api\/me\/profile/, 'checkout 應撈 profile');
  assert.match(src, /prefill/i, '應有 prefill 邏輯註記');
});

test('/me/profile 頁面存在且為 client component（auth gate）', () => {
  const p = path.resolve('app/me/profile/page.tsx');
