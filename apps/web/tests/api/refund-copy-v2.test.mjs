...
describe('refund policy v2 copy sync', () => {
  it('legal/refund page uses refund-v2 exact boundaries (>=168h / >72h<168h / <=72h)', () => {
    ...
    assert.match(src, /出團前\s*72\s*小時以上且少於\s*168\s*小時取消.*?70%/) // >72h <168h
    assert.match(src, /出團前\s*72\s*小時內（含）取消.*?不予退款/) // <=72h
  })
  ...
  it('home FaqSection keeps v2 wording', () => {
    ...
    assert.match(src, /出團\s*168\s*小時前（含）以上可全額退款.*?70%/) // contains 168h boundary + 70%
    assert.match(src, /出團前\s*72\s*小時以上且少於\s*168\s*小時可退\s*70%/) // >72h <168h
  })
})
