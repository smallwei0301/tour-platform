...
test('admin 詳情頁：以 CSRF header POST，並以同 realm fail-closed helper 使用 API canonical redirect', () => {
  const src = readFileSync(ADMIN_PAGE, 'utf8');
  ...
  assert.match(src, /import\s*\{[^}]*resolveGuideLoginRedirect[^}]*\}\s*from\s*['"][^'"]*midao\/login-redirect['"]/, '需匯入同 realm fail-closed redirect helper');
  assert.match(
    src,
    /resolveGuideLoginRedirect\(json\.data\.redirectTo, target\)/,
    '成功後需以同 realm fail-closed helper 使用 API canonical redirect',
  );
});
