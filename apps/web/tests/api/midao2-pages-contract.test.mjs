...
test('midao2 服務列表＋精靈：三步/成交方式/上傳與送審串接', async () => {
  ...
  assert.match(list, /priceFromTwd/);                       // 卡片價格改用起價
...
test('公開接案頁：RSC 直呼領域檔＋表單 honeypot/聯絡驗證/成功畫面', async () => {
  ...
  assert.match(form, /priceFromTwd/);       // 服務卡價格改用起價
  assert.match(form, /g-plan-/);            // 選方案 radio 膠囊列
  assert.match(form, /g-plan-none/);        // 可不指定方案
});

test('登入動線：/midao2 只在 Midao realm 內放行，舊後台仍可互連', async () => {
  const login = await read('app/(non-locale)/guide/login/page.tsx');
  assert.match(login, /resolveGuideLoginRedirect/);
...
test('admin 導遊詳情頁：一鍵代入 midao2 後台', async () => {
  const page = await read('app/(non-locale)/admin/guides/[guideId]/page.tsx');
  assert.match(page, /admin-enter-midao2/);
  assert.match(page, /'\/midao2'/);
});
