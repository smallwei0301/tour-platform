...
describe('guide/apply 照片上傳（GH-1093 + 照片真上傳）', () => {
  test('所有 file input 都需綁 image MIME accept（GH-1093 本義）', () => {
    ...
    assert.ok(fileInputs >= 3, '需有個人照/封面/活動照三個上傳欄位');
  ...
  test('證件仍不收檔案：誠實說明保留', () => {
    assert.ok(source.includes('請勿在表單中提供證件影本'), '需說明不收證件影本');
    assert.ok(source.includes('嚮導後台'), '需說明上線後可於嚮導後台管理照片');
  });
});
