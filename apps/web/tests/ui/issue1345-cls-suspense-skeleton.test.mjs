...
// #1344 更新：fallback 升級為「有 SSR 資料時 render 真卡片首屏
// （ActivitiesFirstPaint），抓不到資料才退回 ActivitiesSkeleton」。
// CLS 保證不變（FirstPaint 鏡射 skeleton 幾何），且仍不得回退到
// 「載入中⋯」單行字串或 null。
test('/activities Suspense 用 ActivitiesFirstPaint（退路 ActivitiesSkeleton），不是「載入中⋯」單行 fallback', async () => {
  ...
  assert.match(
    src,
    /<ActivitiesFirstPaint activities=\{initialActivities\}/,
    'Suspense fallback 必須在有 SSR 資料時用 <ActivitiesFirstPaint />（#1344）',
  );
  assert.match(
    src,
    /:\s*<ActivitiesSkeleton\s*\/>/,
    'SSR 資料抓不到時必須退回 <ActivitiesSkeleton />',
  );
...
test('/activities/[region] Suspense 也用 ActivitiesFirstPaint（退路 ActivitiesSkeleton，不再是 null）', async () => {
