...
// hero-section-redesign：首頁 hero 改為 motion 影片背景（BoomerangVideoBg），
// 不再使用 unsplash 大圖，因此首頁自己的 image preload 也一併移除 —
// LCP 元素回到 hero 標題文字，比下載 w=1600 大圖更快。
// #1344 的核心契約（root layout 不得全站洩漏 image preload）由上面兩個測試續鎖。
test('首頁 page.tsx 不再 preload 已移除的 hero 大圖（hero 已改為 motion 影片）', async () => {
  const src = await readSrc('app/page.tsx');
  assert.doesNotMatch(
    src,
    /rel=["']preload["'][\s\S]{0,300}?as=["']image["']/,
    '首頁 hero 已無大圖，不應殘留 image preload 浪費頻寬',
  );
  assert.match(
    src,
    /rel=["']preconnect["']\s+href=["']https:\/\/d8j0ntlcm91z4\.cloudfront\.net["']/,
    '首頁應 preconnect hero 影片 CDN，加速影片載入',
  );
});
