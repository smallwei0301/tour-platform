 ...
 * 8.8s 的元兇之一。
 *
 * 首頁自己在 app/[locale]/page.tsx 內已有同一條 preload（React 19 會 hoist 到
...
test('首頁 page.tsx 自己的 hero preload 保留（首頁 LCP 不回歸）', async () => {
  const src = await readSrc('app/[locale]/page.tsx');
  ...
  assert.match(
    ...
    '首頁的 hero 前景 preload 必須留在 app/[locale]/page.tsx — root layout 移除後它是唯一的首頁 LCP 加速來源',
  );
  assert.match(
    src,
    /rel=["']preload["'][\s\S]{0,300}?\/images\/lp\/hero-mountains\.webp/,
    '首頁的 hero 遠景 preload 必須留在 app/[locale]/page.tsx',
  );
});
