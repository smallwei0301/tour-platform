...
// 這裡是讀取端。
//
// 顯示規則（方案為唯一來源；活動層級 itinerary 備援已移除，#admin-plan-revert 後續）：
//   - 未選方案 → 顯示提示「請選擇上方的方案，以獲取行程詳細資訊」。
//   - 已選方案且有填行程介紹 → 顯示該方案站點時間表，標題後標上「{方案名稱}」。
//   - 已選方案但未填行程介紹 → 顯示「此方案尚未提供詳細行程」。
//
// 站點資料相容兩種格式：
...
export function PlanItinerarySection({ plans }: PlanItinerarySectionProps) {
  ...
  if (!selected?.id) {
    ...
    // 已選方案：有填行程介紹用方案的；未填則顯示「此方案尚未提供詳細行程」。
    titleSuffix = planLabel || null;
    if (planSteps.length > 0) {
      body = <ItineraryTimeline steps={planSteps} />;
    } else {
      body = (
        <p className="kkd-itinerary-empty" style={{ color: 'var(--tp-muted)', fontSize: 14, margin: 0 }} role="status">
          {t('planNoItinerary')}
