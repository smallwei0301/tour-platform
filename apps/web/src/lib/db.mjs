...
// #1613 strangler：dashboard 彙總需讀取已搬出領域的資料（正向依賴，無循環——
// 領域檔不靜態 import db.mjs；settlement-ops 對 admin-orders 的需求走呼叫時動態 import）。
import { listGuideApplicationsDb } from './db-guide-applications.mjs';
import { listOperationsTrackingDb } from './db-settlement-ops.mjs';
...
// #1613 strangler：本區塊已整塊搬至 db-settlement-ops.mjs（re-export 維持既有匯入路徑）
export { getSettlementRulesDb, getUnsettledOrdersDb, listOperationsTrackingDb, operationsTrackingCsvDb, operationsTrackingSummaryDb, recordSettlementDb, updateOperationsTrackingDb, updateSettlementRulesDb } from './db-settlement-ops.mjs';

// #1613 strangler：本區塊已整塊搬至 db-guide-applications.mjs（re-export 維持既有匯入路徑）
export { createGuideApplicationDb, listGuideApplicationsDb, updateGuideApplicationStatusDb } from './db-guide-applications.mjs';
...
// #1613 strangler：本區塊已整塊搬至 db-wishlist.mjs（re-export 維持既有匯入路徑）
export { addToWishlistDb, listWishlistDb, removeFromWishlistDb } from './db-wishlist.mjs';
...
// #1613 strangler：本區塊已整塊搬至 db-reschedule.mjs（re-export 維持既有匯入路徑）
export { createRescheduleRequestDb, decideRescheduleRequestDb, listGuideRescheduleRequestsDb, listRescheduleOptionsDb, withdrawRescheduleRequestDb } from './db-reschedule.mjs';


// #1613 strangler：本區塊已整塊搬至 db-booking-approvals.mjs（re-export 維持既有匯入路徑）
export { __resetV2BookingStoreForTest, __seedV2BookingForTest, decideBookingApprovalDb, listGuidePendingApprovalsDb } from './db-booking-approvals.mjs';

// #1613 strangler：本區塊已整塊搬至 db-order-messages.mjs（re-export 維持既有匯入路徑）
export { createOrderMessageDb, listGuideMessageThreadsDb, listOrderMessagesDb } from './db-order-messages.mjs';

// #1613 strangler：本區塊已整塊搬至 db-homepage-featured.mjs（re-export 維持既有匯入路徑）
export { getHomepageFeaturedDb, setHomepageFeaturedDb } from './db-homepage-featured.mjs';

// #1613 strangler：本區塊已整塊搬至 db-messaging-bindings.mjs（re-export 維持既有匯入路徑）
export { consumeGuideBindCodeDb, consumeLineBindCodeDb, consumeTelegramBindCodeDb, createGuideBindCodeDb, createLineBindCodeDb, createTelegramBindCodeDb, getGuideBindingDb, getGuideIdForOrderDb, getLineMappingByLineUserIdDb, getLineUserIdForGuideDb, getLineUserIdForOrderDb, getNotificationOverridesDb, getTelegramChatForGuideDb, getTelegramChatForTravelerDb, markLineWebhookEventDb, markTelegramUpdateDb, setGuideLineBlockedDb, setLineBlockedDb, setNotificationCellsDb, setTelegramBlockedDb, upsertGuideLineMappingDb, upsertLineMappingDb, upsertTelegramMappingDb } from './db-messaging-bindings.mjs';
