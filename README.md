...
Phase 6 導遊儀表板       ░░░░░░░░░░░░   0%  🔜 下一個 Sprint
整體完成度：~90%（核心交易閉環已完整，待導遊自助功能 + 正式付款）
```

**本週重大里程碑（2026-04-01）🎉**
- ✅ Admin 行程編輯頁全面翻新：GuideSearch、圖片上傳（Storage）、方案管理、批次排程
- ✅ 方案詳情 Modal（7 tabs：亮點/費用/行程/集合地點/體驗地點/購買須知/取消政策）
- ✅ 圖片 Carousel：手機 swipe 左右切換 + 桌機 3:1 grid
- ✅ JSON 行程匯入/匯出（含欄位驗證 + diff 預覽）
- ✅ **付款 → 訂位扣量完整閉環**：`fn_book_schedule` 原子 RPC + booking page 讀 DB + ISR 即時更新
- ✅ **E2E 測試 8/8 ALL PASS**（Judy QA，2026-04-01，見 `docs/04-tech/03-dev-timeline/e2e-test-report-2026-04-01.md`）
...
| 任務 | 說明 | 優先 |
|------|------|------|
| 導遊登入（JWT / 邀請碼） | 導遊自助管理帳號 | P0 |
| 導遊場次管理 | 導遊可開啟/關閉特定日期的方案 | P0 |
| 導遊訂單查看 | 看到自己場次的報名狀況 | P0 |
| 場次容量自定義 | 導遊可調整每場人數上限 | P1 |
