-- issue #197: sync orders.payment_status with callback-paid state
-- goals:
-- 1) when callback transitions order to paid, set orders.payment_status='paid'
-- 2) on idempotent replay for already-paid/confirmed/completed orders,
--    heal stale rows where payment_status is not paid
-- 3) keep callback path idempotent and compatible with existing return contract
