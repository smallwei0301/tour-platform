-- Issue #1855: fix midao atomic functions that wrongly schema-qualified NULLIF
--
-- Root cause: NULLIF is a PostgreSQL syntactic special form; no same-named
-- function exists in pg_catalog. Once written with the pg_catalog schema
-- prefix, every call raises
--   ERROR: function ... nullif(text, unknown) does not exist
-- at runtime.
--
-- Affected live functions (production read-only evidence, 3 functions / 17 sites):
--   F1 public.midao_publish_service_draft(uuid, integer, uuid)                                   -- 7
--   F2 public.midao_restore_service_publication(uuid, integer, text, text, text, text)           -- 7
--   F3 public.midao_replace_global_day_availability(uuid, date, text, bigint, jsonb, text, text) -- 3
--
-- Function bodies are copied verbatim from production pg_get_functiondef(oid);
-- the only change is the mechanical removal of the wrong schema prefix on the
-- nullif calls. Everything else is byte-identical, including
-- pg_catalog.btrim / pg_catalog.lower, which are real functions and are kept.
--
-- CREATE OR REPLACE keeps the function OID, so existing ACL/GRANT are preserved.
-- This file intentionally issues no privilege statements and no function drops,
-- and does not change signatures, SECURITY DEFINER, or search_path.
--
-- Rollback: 20260819210000_issue1855_pg_catalog_nullif_repair.rollback.sql
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      NULLIF(pg_catalog.btrim(question.elem ->> 'label'), ''),
      question.elem ->> 'question_key'
    ),
    question.elem ->> 'type',
...
DECLARE
  ...
  v_idempotency_key text := NULLIF(pg_catalog.btrim(p_idempotency_key), '');
  v_request_hash text := pg_catalog.lower(NULLIF(pg_catalog.btrim(p_request_hash), ''));
