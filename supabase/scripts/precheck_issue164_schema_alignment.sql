-- Issue #164 precheck: booking/order/payment validation aligned to actual production schema.
-- Canonical relationships:
-- 1) bookings.order_id -> orders.id
-- 2) orders.booking_id -> bookings.id
-- 3) payments.order_id -> orders.id
-- NOTE: payments.booking_id is NOT part of this schema contract.

-- A) Column presence checks for canonical links
SELECT
  EXISTS (
...
-- B) Referential integrity checks for canonical links
SELECT
...
-- C) Data profile snapshot required by #164
SELECT
