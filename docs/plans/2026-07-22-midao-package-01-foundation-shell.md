...
`HOME`為runner-owned 0700 isolated temp dir；`mkdtemp`成功後必須立即進入涵蓋所有npmrc/cache setup、spawn、log與postflight的`try/finally`。Runner以`open(..., 'wx', 0o600)`／`O_CREAT|O_EXCL`建立不同的`path.join(tempHome, 'user.npmrc')`與`path.join(tempHome, 'global.npmrc')`，在仍開啟的FD上`fchmod(0o600)`，再以FD `fstat`驗regular、size 0、mode exact0600；path `lstat`必須非symlink、regular、mode0600，且dev/inode與FD一致。兩path必須不同且皆非`/dev/null`，分別設為`npm_config_userconfig`與`npm_config_globalconfig`；`npm_config_cache=path.join(tempHome, 'npm-cache')`，update-notifier/fund/audit固定false。Adversarial tests必須注入restrictive process umask、exclusive open／fchmod／fstat／lstat失敗、path replacement、第二個npmrc或cache partial-setup失敗，證明spawn前fail closed；另以exact strict child env執行local `npm --version` no-network probe，必須回pinned npm 11.9.0。
