...
- **現行版本**：`supabase/migrations/20260423194000_issue195_callback_booking_status_loop.sql`
  （`fn_process_payment_callback_atomic`，#195 加入 booking status loop 閉合）
- **前一版本**：`supabase/migrations/20260420143000_issue59_payment_race_hardening.sql`（#59 併發加固）
...
| 2. booking loop 閉合 | booking `FOR UPDATE` → `draft → pending_confirmation` + `booking_status_logs` 冪等插入（`WHERE NOT EXISTS`） | L53-98 |
