...
| Admin refund actions (`approve/process/complete/reject`) | admin operator | refund lifecycle transitions | `refund_requests`, `orders`, `payment_events`, `audit_logs` | `requested -> approved -> processing -> refunded` (or rejected) | `admin` / admin-manual channel | Required by #170 | action timestamps | MVP landed (with idempotent refunded event guard) | Routes: `apps/web/app/api/admin/refund-requests/[refundRequestId]/*/route.ts`; logic: `apps/web/src/lib/db.mjs`, `apps/web/src/lib/admin.mjs`; tests: `apps/web/tests/api/admin-refunds.test.mjs`, `apps/web/tests/api/admin-audit-coverage.test.mjs`; cross-entry artifact: `reports/issue-257/admin-manual-cross-entry-consistency-report.md`（#257 / PR #258） | Child path 已補齊，剩 parent-level convergence decision |
| Booking status write-operation (`POST /api/admin/orders/:orderId`) | admin operator | booking status transition write (`booking_status` change + optional note) | `orders` (`booking_status`), `audit_logs` | e.g. `pending_confirmation -> confirmed` / `confirmed -> completed` / `* -> canceled` with before/after retained in audit payload | `admin` (admin dashboard/manual ops) | Required by #170 field contract for write-chain traceability | order update time + audit log `created_at` | MVP landed | Route usage: `apps/web/app/admin/orders/page.tsx`; API behavior/tests: `apps/web/tests/api/admin-orders-update.test.mjs` (status update + audit logging assertions); audit pack: `docs/qa/issue-259-booking-status-manual-transition-audit-pack.md`（#259 / PR #260, commit `63e815613c945762f8dfd4f4346b33d65f6a826f`） | Path 證據已落地，剩 parent #171 最終 GO/HOLD gate |
...
1. **已落地證據存在**：#170（契約基線）、#171（parent checklist）、#236（payment-init）、#257（admin-manual cross-entry）、#259（booking status manual transition）與既有 API/DB 測試證據已覆蓋主要 critical paths。
2. **parent gate 現況**：跨路徑收斂 artifact 已補上，但 parent #171 是否可關閉仍取決於 checklist truth table（目前 booking/create 仍 partial）。
3. **Issue #165 的定位**：維持 truth artifact；不重開 child implementation scope，只反映 repo/GitHub 已落地事實。
