...
| Booking status write-operation (`POST /api/admin/orders/:orderId`) | admin operator | booking status transition write (`booking_status` change + optional note) | `orders` (`booking_status`), `audit_logs` | e.g. `pending_confirmation -> confirmed` / `confirmed -> completed` / `* -> canceled` with before/after retained in audit payload | `admin` (admin dashboard/manual ops) | Required by #170 field contract for write-chain traceability | order update time + audit log `created_at` | MVP landed；later 需 parent-level consolidated runtime proof | Route usage: `apps/web/app/admin/orders/page.tsx`; API behavior/tests: `apps/web/tests/api/admin-orders-update.test.mjs` (status update + audit logging assertions) | Judy blocker item: matrix now has distinct booking-status row; remaining gap is parent #171 consolidated query pack for runtime completeness（follow-up: #171 child slice） |
