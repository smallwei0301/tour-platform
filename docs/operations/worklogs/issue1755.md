...
第二輪focused correction review在 `fedeb38a`仍FAIL/HOLD：A1兩個blocks缺fail-fast，且npm 11.9.0不能將user/global config同設 `/dev/null`。完整A1已合併為單一 `set -euo pipefail` block，temporary HOME內建立不同0600 user/global npmrc。第三輪local-only SPEC與QUALITY/SECURITY/EXECUTABILITY reviewers在 `43975818`雙PASS、blocking 0。第一次exact runtime仍因npm 11 Arborist改寫既有`yarn.lock`而FAIL；source確認load/save行為，original lock已restore且worktree clean。新增fail-safe Yarn quarantine correction須focused PASS後才可重跑；A1 runtime exit 0前仍HOLD。
