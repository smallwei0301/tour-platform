...
> 最後更新：2026-08-12 11:27 CST｜負責 session：tp-builder-fix / gpt-5.6-terra
...
- 允許檔案：依 Kanban `t_5c80c12d` 指定的六個路徑。
...
- 基線 commit：`496c0413781e477abc163b148d01678233a4af69`
...
- 實作 commit：`2055cbb31acaadd3fe59f1d2ad39f1c4af96715b`（base `496c0413781e477abc163b148d01678233a4af69`）。
...
- 不修改 `scripts/toolchain/tp-node22.sh`、provision script 或 GitHub Actions：canonical root 的 fail-closed preflight 是既定正確行為；本卡只分離 host-bound runtime tests。
- 不將 runtime contract 放回 ordinary 或 portable infrastructure：此組態會在 GitHub Actions 缺少 canonical root 時重現原始失敗。
