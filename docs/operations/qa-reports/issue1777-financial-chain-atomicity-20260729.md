...
| 6 | 已結算後部分退款，最終導遊淨應付等於未退款有效金額 × 導遊分潤率 | **PASS** | `issue1777-refund-adjustment.test.mjs` 情境 2：結算 8500 → 退 3000 → 最終 5950 = `floor(7000 × 0.85)`，且與「結算前就退款」結果相同 |
| 7 | 多次部分退款、callback／refund 重送具冪等性；每個事件最多記一次差額 | **PASS** | 冪等鍵 `${orderId}:cum:${累積退款額}`；測試驗證重送同鍵／二次退款新鍵／分次退與一次退到底等值 |
...
| Fresh independent review | **待執行** | issue Verification 明列需獨立 review（安全／帳務不變量／migration 可回滾／測試不可只驗 source string）。本報告由實作者撰寫，**不能取代獨立審查**。 |
